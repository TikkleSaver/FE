...
const ExpenseSection = () => {
  ...
  const dummyAmounts = [
    50000, 40000, 60000, 30000, 45000, 35000, 55000, 47000, 62000, 51000, 58000,
    39000,
  ];

  // 현재 달까지는 원래 값, 이후는 0으로 처리
  const dummyBarData = Array.from({ length: 12 }, (_, i) => ({
    name: `${i + 1}월`,
    amount: i <= currentMonth ? dummyAmounts[i] : 0,
  }));
  const dummyPieData = categories.map((category) => ({
    name: category.label,
    value: (() => {
      switch (category.id) {
        case 1:
  ...
  useEffect(() => {
    if (!isLoggedIn) {
      ...
      setCurrentMonthExpense(120000);
      setSaveExpense(5000);
      setAchievedDays(15);
      setAchievementRate(50);
      return;
    }
